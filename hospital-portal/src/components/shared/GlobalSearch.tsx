...
export const GlobalSearchProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const { data: patients = [] } = useQuery({
    ...
    queryFn: async () => {
      const rows = await hospitalApi.patientsSearch(query || " ") as Record<string, unknown>[];
      return rows.map(mapSearchPatient);
    },
    enabled: open,
  });
  ...
  return (
    <SearchCtx.Provider value={{ open, setOpen }}>
      ...
      <CommandDialog open={open} onOpenChange={setOpen}>
        <CommandInput placeholder="Search patients, ID, phone, national ID…" value={query} onValueChange={setQuery} />
        <CommandList>
              ...
              <CommandItem key={p.id} value={`${p.name} ${p.id} ${p.phone} ${p.nationalId || ""}`} onSelect={() => go(`/patients/${p.id}`)}>
                <div className="flex items-center gap-sm w-full">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-xs text-text-secondary truncate">{p.id.slice(0, 8)}… · {p.phone}</div>
                  </div>
                </div>
              </CommandItem>
            ))}
